...
def test_integrator_code():
    ...
    for integrator, lines in zip([linear, euler, rk2, rk4], [2, 2, 3, 6]):
        ...
        err_msg = (
            f"Returned code for integrator {integrator.__class__.__name__} had"
            f" {len(code_lines)} lines instead of {int(lines)}"
        )
    ...
    for varname in ["x", "f", "g"]:
        ...
        for integrator in [linear, euler, rk2, rk4]:
            ...
            code_var = re.sub(rf"\b{varname}\b", f"{varname}0", code_var)
            code_var = re.sub(rf"\b(\w*)_{varname}\b", rf"\1_{varname}0", code_var)
        ...
        try:
            ...
            if able is False:
                raise AssertionError(
                    "Should not be able to integrate these "
            ...
            if able is True:
                raise AssertionError(
                    "Should be able to integrate these "
                    f"equations (equations: '{eqs}') with "
                    f"integrator {integrator.__class__.__name__}"
                )
...
def test_subexpressions():
    ...
    methods = [
        ...
        "rk2",
        "rk4",
    ]  # euler is tested in test_subexpressions_basic
...
def test_locally_constant_check():
    ...
    for ta_func, ok in zip([ta0, ta1, ta2, ta3], [True, True, False, False]):
        ...
        # multiplicative
        G = NeuronGroup(
            1, "dv/dt = -v*ta(t)/(10*ms) : 1", method="exact", namespace={"ta": ta_func}
        )
        net = Network(G)
        if ok:
            # This should work
    ...
    G = NeuronGroup(
        1, "dv/dt = -v*ta(t/2.0)/(10*ms) : 1", method="exact", namespace={"ta": ta0}
    )
    net = Network(G)
    with pytest.raises(BrianObjectException) as exc:
        net.run(0 * ms)
    ...
    G = NeuronGroup(1, "dv/dt = -v/(10*ms) + sin(2*pi*100*Hz*t)*Hz : 1", method="exact")
    net = Network(G)
    with pytest.raises(BrianObjectException) as exc:
        net.run(0 * ms)
    assert exc_isinstance(exc, UnsupportedEquationsException)
    ...
    G = NeuronGroup(
        1, "dv/dt = -v/(10*ms) + sin(2*pi*100*Hz*5*second)*Hz : 1", method="exact"
    )
    net = Network(G)
    net.run(0 * ms)
...
def test_refractory():
    ...
    for method in [
        ...
        G_no_ref.v = "(i+1)/11."
        G_ref = NeuronGroup(
            10, f"{eqs_base}(unless refractory)", refractory=1 * ms, method=method
        )
        G_ref.v = "(i+1)/11."
        ...
        assert_allclose(
            G_no_ref.v[:],
            G_ref.v[:],
            err_msg="Results with and without refractoriness differ for method %s."
            % method,
        )
